<?php
    ...
    if(isset($_SESSION['user'])){
        ...
        if(isset($_GET['tour'])){
         ...
         else if(isset($_GET['asem-contact'])){
            $template = $twig->loadTemplate('contactuseng.html');
        ...
        if(isset($_GET['register'])){
            ...
            $max = $e->get('min_rate'); 
            //render a template
            echo $template->render(array('title' => 'Хайлт', 'nav' => 1, 'location' => $location, 'results' =>$results,'start' => $checkin, 'end' => $checkout, 'count' => $count, 'country' => 'Mongolia', 'pages' => $pages, 'guests'=>$guests, 'persons'=>$persons, 'max'=>$max, 'min'=>$min));
        }
        else if(isset($_POST['mapGeoId'])){
            $query = new ParseQuery("hotel");
            $query->equalTo("objectId",$_POST['mapGeoId']);
