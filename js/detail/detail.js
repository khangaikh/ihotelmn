...
$(document).ready(function() {
    ...
    $(document).on('change','select[name=qty]',function(e){
        ...
        if($(this).val()>0){
            ...
            $("#payment_rooms").append('<ul name="order_room" roomid="'+roomid+'"class="booking-item-payment-price"><li><p class="booking-item-payment-price-title">'+room_name+'('+persons+')</p><p class="booking-item-payment-price-title" style="margin-left:10px"><span name="qtys">'+$(this).val()+'</span><small>/өрөө</small></p><p class="booking-item-payment-price-amount">$<span name="subtotal">'+subtotal+'</span><small>/хоногийн</small></p></li></ul>'
                );
            set_total(days);
        }else{
            var roomid = $(this).attr('roomid');
